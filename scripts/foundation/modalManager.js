window.modalManager =
{
	...
	openModal: function(modalID)
	{
		var $modal = $('#' + modalID),
			...
			fadeOverlayIn = function()
			{
				...
				utilityFunctions.setTransitionListeners($overlay[0], fadeInModalFunction);
			},
			fadeInModalFunction = function()
			{
				...
				utilityFunctions.removeTransitionListeners($overlay[0], fadeInModalFunction);
			};
	...
	initiateModalListeners: function()
	{
		...
		$modals.each(function()
		{
			...
			$this.find('.hasExitButton').on('click', view.closeModal);
		});
		...
		$modals.find('.navButton').on('click', { view : view }, view.handleAction);
	},
	...
	{
		var bodyOverlay, strongerBodyOverlay;

		if ($('.modal').length)
		{
			bodyOverlay = document.createElement('DIV');
			bodyOverlay.className = this.MODAL_OVERLAY_CLASS;

			strongerBodyOverlay = document.createElement('DIV');
			strongerBodyOverlay.className = this.MODAL_OVERLAY_STRONGER_CLASS;
	...
	initiateModals: function()
	{
		$('.content').find('.modal').each(function()
		{
			...
			// All modal elements must be initially unrendered
			$this.hide();
		...
		// Always hide the nested modal upon page load
		$('.nestedModal').hide();

		this.initiateModalListeners();
	}
};